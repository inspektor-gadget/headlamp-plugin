...
export function prepareFilters(params, operatorParamsCollection, pods, namespaces) {
  ...
  const allowedNamespaces = (
    JSON.parse(localStorage.getItem(`clusters_settings.${Utils.getCluster()}`)) || {}
  ).allowedNamespaces;
  if (params) {
          ...
          component: () => {
            ...
            return (
              <FormControl variant="filled" fullWidth label={item.title}>
                <InputLabel htmlFor={`${item.title}`}>{item.title}</InputLabel>
                <Select
                  value={value}
                  onChange={event => {
      ...
      if (namespaces && param.valueHint.split(':')[1] === 'namespace') {
          ...
          component: () => {
            ...
            return (
              <FormControl fullWidth variant="filled">
                <InputLabel htmlFor="namespace-select">Namespace</InputLabel>
                <Select
                  defaultValue="All"
                  value={value}
                  onChange={event => {
                ...
                >
                  ...
                  {(allowedNamespaces || namespaces).map(namespace => {
                    if (allowedNamespaces) {
                      return <MenuItem value={namespace}>{namespace}</MenuItem>;
                    }
                    return (
                      <MenuItem value={namespace.metadata.name}>{namespace.metadata.name}</MenuItem>
                    );
                  })}
                </Select>
              </FormControl>
        ...
        filters[`operator.${key}.${param.key}`] = {
          ...
          component: () => {
            ...
            return (
              <FormControl fullWidth variant="filled">
                <InputLabel htmlFor="podname-select">Podname</InputLabel>
                <Select
                  defaultValue="All"
                  value={value}
                  onChange={event => {
                ...
                >
                  <MenuItem value="">All</MenuItem>
                  {pods.map(pod => {
                    return <MenuItem value={pod.metadata.name}>{pod.metadata.name}</MenuItem>;
                  })}
                </Select>
              </FormControl>
            );
      ...
      switch (param.type) {
        ...
        case 'bool':
          filters[`operator.${key}.${param.key}`] = {
            ...
            component: (props: {}) => {
              const [value, setValue] = React.useState(
                JSON.parse(filters[`operator.${key}.${param.key}`].value)
              );
              return (
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={event => {
                        ...
                        filters[`operator.${key}.${param.key}`].value = String(
